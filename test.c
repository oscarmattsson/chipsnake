...
void test_update(int* buttons, int* switches) {
  ...
  if(buttons[2]) {
    ...
    for(i = 0; i < HIGHSCORE_ENTRIES; i++) {
      ...
      i2c_send(EEPROM_MEM_ADD + i*HIGHSCORE_LENGTH + 3);
      i2c_restart();
      i2c_send(EEPROM_READ);
      test_scores[i] = (i2c_recv() - 48) * 10000;
      i2c_ack();
      test_scores[i] += (i2c_recv() - 48) * 1000;
      i2c_ack();
      test_scores[i] += (i2c_recv() - 48) * 100;
      i2c_ack();
      test_scores[i] += (i2c_recv() - 48) * 10;
      i2c_ack();
      test_scores[i] += (i2c_recv() - 48);
      i2c_stop();
    }
  }
  if(buttons[1]) {
...
void test_draw(void) {
  ...
  insert_num(1, 1, test_scores[0], menufield, 1);
  insert_num(1, 7, test_scores[1], menufield, 1);
  insert_num(1, 13, test_scores[2], menufield, 1);
  insert_num(1, 19, test_scores[3], menufield, 1);
  insert_num(1, 25, test_scores[4], menufield, 1);

  insert_num(64, 1, test_scores[5], menufield, 1);
  insert_num(64, 7, test_scores[6], menufield, 1);
  insert_num(64, 13, test_scores[7], menufield, 1);
  insert_num(64, 19, test_scores[8], menufield, 1);
  insert_num(64, 25, test_scores[9], menufield, 1);
